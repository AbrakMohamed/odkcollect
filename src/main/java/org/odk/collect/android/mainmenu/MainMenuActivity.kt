...
class MainMenuActivity : LocalizedActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Ajoutez la vérification de connexion ici
        if (!isLoggedIn()) {
            redirectToLogin()
        }
    }
    ...
    private fun isLoggedIn(): Boolean {
        // Implémentez ici la logique pour vérifier si l'utilisateur est connecté
        // Par exemple, vérifiez si une session valide est active
        // Vous pouvez utiliser des préférences partagées, une base de données locale, etc.
        // Retourne true si l'utilisateur est connecté, sinon false
        // Exemple simplifié :
        return true // Ici, retourne true si l'utilisateur est connecté
    }

    private fun redirectToLogin() {
        // Redirigez l'utilisateur vers LoginActivity pour se connecter
        val intent = Intent(this, LoginActivity::class.java)
        startActivity(intent)
        finish() // Optionnel : terminez cette activité pour empêcher l'utilisateur de revenir ici en appuyant sur Retour
    }
